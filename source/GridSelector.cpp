...
GridSelector::GridSelector(int ix, int iy, int iw, int ih, int iD) {
    x = ix;
    y = iy;
    w = iw;
    h = ih;
...
int GridSelector::idtor(int i, int d, int* ox, int* oy) const {
    int out = 1;
    if((i>=0)&&(i<(4*d))&&(d>=0)) {
...
int GridSelector::N(int d) const {
    int out = (d?4*d:1);
    return out;
}
